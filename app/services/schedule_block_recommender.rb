class ScheduleBlockRecommender
  ...
  def create_possible_recommendations_for_interview
    recommendations_count = interview.possible_interview_blocks.count

    until recommendations_count == 25
    end
  end

# 2) break up next day into 30 min blocks. Chronic.parse("tomorrow at 7:00")...
# 3) CYCLE -- every 30 min block |block|
